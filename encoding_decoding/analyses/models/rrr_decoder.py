...
class RRRGD():
    def __init__(self, train_data, ncomp, l2=0.):
        ...
        np.random.seed(0)
        self.ncoef = 0; self.model = {}
        for eid in train_data:
            _X = train_data[eid]['X'][0] # (K,T,N)
            _y = train_data[eid]['y'][0] # (K,T,ncoef), the last coef is the bias term
            K, T, N = _X.shape
            K, T, ncoef = _y.shape
            U = np.random.normal(size=(ncoef, N-1, ncomp))/np.sqrt(T*ncomp)
    ...
    * input has to be tensor
    ...
    def predict(self, beta, X, tonp=False):
        ...
        if tonp == True:
            X = np2tensor(X)
            beta = np2tensor(beta)
        y_pred = torch.einsum("ktn,cnt->ktc", X, beta)
        if tonp == True:
    ...
    - output tensor
    """
    def predict_y_behavior(self, data, eid, k):
